...
export async function renderQuiz(container, params) {
  ...
  if (lessonId.startsWith('dict:')) {
    const level = lessonId.split(':')[1];
    lesson = { id: lessonId, title: `${level} レベル 単語クイズ` };

    container.innerHTML = `<div style="text-align:center;padding:var(--space-8);color:var(--text-muted)">クイズを生成中...</div>`;
    try {
      const res = await fetch(`/api/dictionary/quiz?level=${level}&count=10`);
  ...
  container.innerHTML = `
    <div class="animate-scale-in" style="text-align:center;padding:var(--space-8) 0">
      ...
      <div style="display:flex;gap:var(--space-3);justify-content:center;flex-wrap:wrap">
        ...
        ${pct < 80 ? (
      lesson.id.startsWith('dict:')
        ? `<button class="btn btn-accent" onclick="window.navigateTo('vocabulary', '${lesson.id.split(':')[1]}')">単語帳を復習</button>`
        : `<button class="btn btn-accent" onclick="window.navigateTo('lesson', '${lesson.id}')">レッスンを復習</button>`
    ) : ''}
      </div>
    </div>
  `;
