...
const STORAGE_KEY = 'fluencyflow_data';
...
class StorageServiceClass {
    ...
    incrementPronunciation() {
        this.data.pronunciationCount = (this.data.pronunciationCount || 0) + 1;
        this._save();
    }

    incrementWordsLearned(count = 1) {
        this.data.wordsLearned = (this.data.wordsLearned || 0) + count;
        this._save();
    }

    getStudyDays() {
        // Count unique study dates from completed lessons
        const dates = new Set();
        Object.values(this.data.lessonsCompleted).forEach(l => {
            if (l.date) dates.add(l.date.split('T')[0]);
        });
        Object.values(this.data.conversationsCompleted || {}).forEach(c => {
